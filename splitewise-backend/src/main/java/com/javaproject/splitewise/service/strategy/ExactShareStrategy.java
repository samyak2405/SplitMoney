...
public class ExactShareStrategy implements ComputeShareStrategy {
    ...
    public Map<String, String> computeShare(List<ExpenseParticipantRequest> participants, String totalAmount) {
        ...
        for (ExpenseParticipantRequest participant : participants) {
            BigDecimal exactAmount = new BigDecimal(participant.getExactAmount()).setScale(2, RoundingMode.HALF_UP);
            sharesByUser.put(participant.getEmail().trim().toLowerCase(Locale.ROOT), exactAmount);
            runningTotal = runningTotal.add(exactAmount);
        }

        if (runningTotal.compareTo(normalizedTotal) != 0) {
